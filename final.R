...
## load dataframes
# nextstrain
nextstrain<-read.delim("metadata.tsv.gz")
# gisaid
GISAID<-read.delim("gisaid.metadata.tsv.gz")

### organize dataframes
## nextstrain
n<-nextstrain[c(6,28,30)]
names(n)<-c("coldate", "subdate", "variant")
# change pango lineage to WHO nomenclature
n$variant[n$variant=="B.1.1.7"] <- "Alpha"
n$variant[n$variant=="B.1.617.2"] <- "Delta"
n$variant[n$variant=="B.1.1.529"] <- "Omicron"
n$variant[startsWith(n$variant, "Q.")] <- "Alpha"
n$variant[startsWith(n$variant, "B.1.351")]<-"Beta"
n$variant[startsWith(n$variant, "P.1")]<-"Gamma"
n$variant[startsWith(n$variant, "AY.")]<-"Delta"
n$variant[startsWith(n$variant, "BA.")]<-"Omicron"
n$variant[n$variant!="Alpha"&n$variant!="Beta"&n$variant!="Gamma"&n$variant!="Delta"&n$variant!="Omicron"]<-"other"
# sort by count
n<-subset(n, coldate > "2020-09-01")
n$coldate<-as.Date(n$coldate)
n$subdate<-as.Date(n$subdate)
n$variant<-as.character(n$variant)
n<-n%>%group_by(coldate, subdate, variant)%>%mutate(count=n())
n<-n[!duplicated(n),]
## gisaid 
g<-GISAID[c(4,12,16)]
names(g)<-c("coldate", "variant", "subdate")
# change pango lineage to WHO nomenclature 
g$variant[g$variant=="B.1.1.7"] <- "Alpha"
g$variant[g$variant=="B.1.617.2"] <- "Delta"
g$variant[g$variant=="B.1.1.529"] <- "Omicron"
g$variant[startsWith(g$variant, "Q.")] <- "Alpha"
g$variant[startsWith(g$variant, "B.1.351")]<-"Beta"
g$variant[startsWith(g$variant, "P.1")]<-"Gamma"
g$variant[startsWith(g$variant, "AY.")]<-"Delta"
g$variant[startsWith(g$variant, "BA.")]<-"Omicron"
g$variant[g$variant!="Alpha"&g$variant!="Beta"&g$variant!="Gamma"&g$variant!="Delta"&g$variant!="Omicron"]<-"other"
# sort by count
g<-subset(g, coldate > "2020-09-01")
g$coldate<-as.Date(g$coldate)
g$subdate<-as.Date(g$subdate)
g$variant<-as.character(g$variant)
g<-g%>%group_by(coldate, subdate, variant)%>%mutate(count=n())
g<-g[!duplicated(g),]
...
# organize nextstrain dataframe 
n<-nextstrain[c(6,28,30,8)]
names(n)<-c("coldate", "subdate", "variant","country")
...
n<-subset(n, coldate > "2020-09-01")
n<-n%>%group_by(variant, coldate, subdate, country)%>%dplyr::mutate(count=n())
n<-n[!duplicated(n),]

# organize gisaid dataframe
g<-GISAID[c(4,12,16,5)]
names(g)<-c("coldate", "variant", "subdate", "location")
...
a<-as.data.frame(str_split_fixed(g$location, " / ", 3))
names(a)<-c("Continent", "Country", "Region")
g<-data.frame(cbind(g,a))

g$coldate<-as.Date(g$coldate)
g$subdate<-as.Date(g$subdate)

g<-subset(g, coldate > "2020-09-01")
g<-g%>%group_by(variant, subdate, country)%>%dplyr::mutate(count=n())
g<-g[!duplicated(g),]
...
ggplot(n022021a, aes(x=country, y=n))+
...
n032021a<- n032021 %>% 
  ...
  dplyr::summarize(n=n())%>% 
  filter(dense_rank(-n) < 10)
ggplot(n032021a, aes(x=country, y=n))+
  geom_bar(stat="identity")
...
n102021a<- n102021 %>% 
  ...
  dplyr::summarize(n=n())%>% 
  filter(dense_rank(-n) < 10)
ggplot(n102021a, aes(x=country, y=n))+
  geom_bar(stat="identity")
...
n032022a<- n032022 %>% 
  ...
  dplyr::summarize(n=n())%>% 
  filter(dense_rank(-n) < 10)
ggplot(n032022a, aes(x=country, y=n))+
  geom_bar(stat="identity")
...
g122020a<- g122020 %>% 
  ...
  dplyr::summarize(n=n())%>% 
  filter(dense_rank(-n) < 10)
ggplot(g122020a, aes(x=country, y=n))+
  geom_bar(stat="identity")
...
g022021a<- g022021 %>% 
  ...
  dplyr::summarize(n=n())%>% 
  filter(dense_rank(-n) < 10)
ggplot(g022021a, aes(x=country, y=n))+
  geom_bar(stat="identity")
...
g062021a<- g062021 %>% 
  ...
  dplyr::summarize(n=n())%>% 
  filter(dense_rank(-n) < 10)
ggplot(g062021a, aes(x=country, y=n))+
  geom_bar(stat="identity")
...
g012022a<- g012022 %>% 
  ...
  dplyr::summarize(n=n())%>% 
  filter(dense_rank(-n) < 10)
ggplot(g012022a, aes(x=country, y=n))+
  geom_bar(stat="identity")
